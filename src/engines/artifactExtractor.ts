...
export interface DiscoveredArtifact {
  id: string;
  type: 'SLIDE_DECK' | 'CANVAS_DOCUMENT' | 'CANVAS_WIDGET' | 'GENERATED_IMAGE' | 'GENERATED_FILE';
  sessionTitle: string;
  sessionId: string;
  turnIndex: number;
  userPseudoId: string;
...
export class ArtifactExtractor {
  ...
  private extractArtifactsFromAnswer(
    ...
    for (let rIdx = 0; rIdx < replies.length; rIdx++) {
      ...
      // C. Generated Images / Media
      if (content.file) {
        const file = content.file;
        const isImg = file.mimeType?.startsWith('image/');
        results.push({
          id: `file_${sessionId}_t${turnIndex}_${file.fileId || rIdx}`,
          type: isImg ? 'GENERATED_IMAGE' : 'GENERATED_FILE',
          sessionTitle,
          sessionId,
          turnIndex,
          userPseudoId,
          title: `Generated ${isImg ? 'Image' : 'File'} (${file.mimeType || 'unknown'})`,
          description: `Asset ID: ${file.fileId || 'N/A'}`,
          fileMetadata: {
