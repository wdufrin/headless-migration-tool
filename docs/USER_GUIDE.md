...
| :--- | :--- | :--- |
| **`More notebooks discovered than expected (shared colleague notebooks marked as owned)`** | The impersonated user holds project-level `discoveryengine.notebooks.delete` (Admin permissions, e.g. `roles/discoveryengine.admin` on the Workforce Pool or an admin group). In NotebookLM, project-wide delete permission causes Google's API to treat the user as `PROJECT_ROLE_OWNER` on ALL shared notebooks in the project. | Replace `roles/discoveryengine.admin` with `roles/discoveryengine.user` on the Workforce Pool in the Source project:<br>`gcloud projects add-iam-policy-binding <PROJECT> --member="principalSet://iam.googleapis.com/locations/global/workforcePools/<POOL>/*" --role="roles/discoveryengine.user"`<br>`gcloud projects remove-iam-policy-binding <PROJECT> --member="principalSet://iam.googleapis.com/locations/global/workforcePools/<POOL>/*" --role="roles/discoveryengine.admin"` |
